...
Use `$CONF['encrypt'] = 'MD5-CRYPT';` for the canonical format, which includes
the scheme prefix:

`{MD5-CRYPT}$1$c9809462$M0zeLuOvixH61C2csGN.U0`

The legacy `$CONF['encrypt'] = 'md5crypt';` spelling remains supported for
compatibility with PostfixAdmin 3.x configurations. It generates the same
MD5-CRYPT hash without the scheme prefix:

`$1$c9809462$M0zeLuOvixH61C2csGN.U0`

Both settings can verify prefixed and unprefixed MD5-CRYPT hashes. They also
continue to verify bcrypt hashes generated by affected PostfixAdmin 4.x
versions when `md5crypt` was incorrectly treated as `CRYPT`.

MD5-CRYPT is weak by current standards. Do not use it for new installations;
prefer a stronger format such as ARGON2ID when all authentication services
support it.
