 ...
 *
 * @return bool True if they have the requested role in their session.
 ...
 * @param bool $is_admin true if the user is an admin, false otherwise
 *
...
function db_connect($ignore_errors = false) {
    ...
    if ($link) {
        ...
        echo "Connect: Unable to connect to database<br />\n";
        echo "<br />\n";
        echo "Make sure that you have set the correct database type in the config.inc.php file<br />\n";
        echo $DEBUG_TEXT;
        die();
    }
}
...
function db_get_boolean($bool) {
    ...
    if (Config::Read('database_type') == 'mysql' || Config::Read('database_type') == 'mysqli' || db_sqlite()) {
        if ($bool) {
            return 1;
        }
...
function alias_domain_postdeletion($alias_domain) {
    ...
    if (empty($alias_domain)) {
        echo '<p>Warning: empty alias_domain parameter.</p>';
    ...
    if (0 != $retval) {
        ...
        echo '<p>WARNING: Problems running alias_domain postdeletion script!</p>';
