...
class AutoconfigHandler extends PFAHandler 
{
    ...
    public function get_config_ids_for_user( $user )
    {
		...
		$sth = $res['result'];
		if( DEBUG ) error_log( "get_config_ids_for_user() \$sth = " . print_r( $sth, true ) );
    ...
    public function has_permission_over_config_id( $user, $this_config_id )
    {
    	...
    	{
    		if( $this->debug ) error_log( "has_permission_over_config_id() user is empty, or no config was provided." );
    		return( false );
    	}
    	...
    	if( !empty( $row ) )
    	{
    		...
    		if( !$row['active'] )
    		{
				if( $this->debug ) error_log( "has_permission_over_config_id() config $this_config_id is not active." );
    			return( false );
    		}
    		// Admin is a super admin, so he has access to everything
    		elseif( $row['superadmin'] )
    		{
				if( $this->debug ) error_log( "has_permission_over_config_id() user $user is a super admin, returning true." );
    			return( true );
    		}
    		else
    		{
				...
				if( $this->debug ) error_log( "has_permission_over_config_id() checking user '$user' permission over config '$this_config_id' with sql query: $sql" );
    			$res = db_query( $sql );
				if( !empty( $res['error'] ) )
				{
					$this->error = $res['error'];
    ...
    public function config_id( $id )
    {
    	if( isset( $id ) )
    	{
    		if( $this->debug ) error_log( "config_id() checking config id \"$id\"." );
			$table_autoconfig = table_by_key('autoconfig');
			...
			if( empty( $row ) ) 
			{
				if( $this->debug ) error_log( "config_id() could not find config id \"$id\"." );
				return( false );
			}
    		if( $this->debug ) error_log( "config_id() config id \"$id\" found." );
    ...
    {
    	if( empty( $sth ) ) throw( "No statement handler was provided." );
    	try
    	{
    ...
    public function get_config_ids()
    {
		...
		if( $this->debug ) error_log( "get_config_ids() returning '" . print_r( $list, true ) . "'" );
		return( $list );
    }
    ...
    private function get_domains( $id )
    {
		...
		if( $this->debug ) error_log( "get_domains() executing following query to get the list of authorised domains: $sql" );
		$res = db_query( $sql );
		if( !empty( $res['error'] ) )
		{
			$this->error = $res['error'];
		...
		if( $this->debug ) error_log( "get_domains() returning '" . print_r( $list, true ) . "'" );
		return( $list );
    }
    ...
    public function get_details( $id ) 
    {
    	...
    	if( empty( $id ) )
    	{
    		if( $this->debug ) error_log( "No id provided, returning false." );
    		return( false );
    	}
    	...
    	{
    		if( $this->debug ) error_log( sprintf( "Admin %s has no permission over this config %s, returning false.", $this->username, $id ) );
    		return( false );
    	}
    	$config = [];
    	$conf_domains = [];
    	...
    	{
    		if( $this->debug ) error_log( "Unable to get basic config data, returning false." );
    		return( false );
    	}
    	// If active is null, set it to true by default
    	if( is_null( $config['active'] ) ) $config['active'] = true;
    	...
    	{
    		if( $this->debug ) error_log( "Unable to get config domains, returning false." );
    		return( false );
    	}
    	if( $this->debug ) error_log( sprintf( "Found %d domains for config $id", count( $conf_domains ) ) );
    	$config['provider_domain'] = $conf_domains;
    	// To build the domain names html menu
    	$config['provider_domain_options'] = $this->all_domains;
    	if( $this->debug ) error_log( sprintf( "Found %d total domains for config $id", count( $conf_domains ) ) );
    	
    	// Get incoming servers
    	if( ( $conf_hosts_in = $this->get_hosts( 'in', $id ) ) === false )
    	{
    		if( $this->debug ) error_log( "Unable to get config incoming hosts, returning false." );
    		return( false );
    	}
    	if( $this->debug ) error_log( sprintf( "Found %d incoming servers for config $id", count( $conf_hosts_in ) ) );
    	...
    	for( $j = 0; $j < count( $conf_hosts_in ); $j++ )
    	{
    		...
    		if( $this->debug ) error_log( "get_details(): incoming host data is now: " . print_r( $conf_hosts_in[$j], true ) );
    	}
    	...
    	{
    		if( $this->debug ) error_log( "Unable to get config outgoing hosts, returning false." );
    		return( false );
    	}
    	if( $this->debug ) error_log( sprintf( "Found %d outgoing servers for config $id", count( $conf_hosts_out ) ) );
    	...
    	{
    		if( $this->debug ) error_log( "Unable to get enabling instrucctions, returning false." );
    		return( false );
    	}
    	if( $this->debug ) error_log( sprintf( "Found %d enable instruction(s) for config $id", count( $conf_texts_inst ) ) );
//     	$langs = array();
...
//     	}
    	...
    	{
    		if( $this->debug ) error_log( "Unable to get documentation description, returning false." );
    		return( false );
    	}
    	if( $this->debug ) error_log( sprintf( "Found %d documentatoin description(s) for config $id", count( $conf_texts_doc ) ) );
//     	$langs = array();
...
//     	}
    	...
    	if( $this->debug ) error_log( sprintf( "Returning hash ref with %d keys", count( array_keys( $config ) ) ) );
    	return( $config );
    }
    ...
    public function remove_config( $id )
    {
		...
		{
			if( DEBUG ) error_log( "remove_config(): An error occurred while trying to remove config id $id: " . $e->getMessage() );
			$this->error = $e->getMessage();
			return( false );
		}
		return( $ok );
    ...
    public function save_config( &$data )
    {
    	...
    	elseif( !empty( $data['config_id'] ) )
    	{
			...
			{
				if( $this->debug ) error_log( sprintf( "save_config() config id submitted \"%s\" is not the same as our current id \"%s\"", $data['config_id'], $this->config_id ) );
    	...
    	if( DEBUG ) error_log( "Base config data are: " . print_r( $config_data, true ) );
    	...
    	try
    	{
			...
			if( !$is_new )
			{
				if( $this->debug ) error_log( "save_config() get current domain names for this update." );
			...
			if( !empty( $this->config_id ) )
			{
				if( $this->debug ) error_log( "save_config() get current domain names for this config id \"" . $this->config_id . "\"." );
				if( ( $current_domains = $this->get_domains( $this->config_id ) ) === false )
				{
					if( $this->debug ) error_log( "save_config() get_domains returned: '" . print_r( $current_domains, true ) . "'." );
					$this->error = $PALANG['pAutoconfig_no_domain_authorised_for_this_admin'];
					db_rollback();
					return( false );
				}
			...
			foreach( $textTypes as $textType )
			{
				...
				{
					if( $this->debug ) error_log( "save_config() No lang found for text $textType" );
				}
