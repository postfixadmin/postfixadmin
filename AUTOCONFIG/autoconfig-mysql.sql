...
CREATE TABLE IF NOT EXISTS autoconfig_domains (
    id                                INTEGER AUTO_INCREMENT,
    config_id                         CHAR(36) NOT NULL,
    -- COLLATE may need specifying here if the target has one ...
    domain                            VARCHAR(255) NOT NULL,
    PRIMARY KEY (id),
    UNIQUE KEY (config_id, domain),
    FOREIGN KEY (domain) REFERENCES domain(domain) ON DELETE CASCADE,
    FOREIGN KEY (config_id) REFERENCES autoconfig(config_id) ON DELETE CASCADE
) ENGINE = InnoDB;
