...
if ($f_phpversion == 1) {
    if (phpversion() < 5) {
        ...
        $phpversion = 5;
        echo '<li><b>Recommended PHP version: >= 5.2.3, you have ' . phpversion() . "</b></li>\n";
    } else {
        $phpversion = 5;
...
if ($f_get_magic_quotes_gpc == 1) {
    if (get_magic_quotes_gpc() == 0) {
        echo "<li>Magic Quotes: Disabled - OK</li>\n";
    } else {
        echo "<li><b>Warning: Magic Quotes: ON (internal workaround used)</b></li>\n";
    }
} else {
    echo "<li><b>Unable to check for Magic Quotes. (missing function: get_magic_quotes_gpc())</b></li>\n";
}
...
$config_loaded = 0;
if ($file_config == 1) {
    echo "<li>Depends on: presence config.inc.php - OK</li>\n";
    require_once $incpath . '/config.inc.php';
    $config_loaded = 1;

    if (isset($CONF['configured'])) {
        if ($CONF['configured'] === true) {
            echo "<li>Checking \$CONF['configured'] - OK\n";
        } else {
            echo "<li><b>Warning: \$CONF['configured'] is 'false'.<br>\n";
            echo "You must edit your config.local.php and change this to true (this indicates you've created the database and user)</b>\n";
        }
    }
} else {
    echo "<li><b>Error: Depends on: presence config.inc.php - NOT FOUND</b><br /></li>\n";
...
if ($file_local_config == 1) {
    echo "<li>Depends on: presence config.local.php - OK</li>\n";
} else {
    echo "<li><b>Warning: config.local.php - NOT FOUND</b><br /></li>\n";
    echo "It's Recommended to store your own settings in config.local.php instead of editing config.inc.php<br />";
    echo 'Create the file, and edit as appropriate (e.g. select database type etc)<br />';
}
...
if (($f_mysql_connect == 0) and ($f_mysqli_connect == 0) and ($f_pg_connect == 0) and ($f_sqlite_open == 0)) {
    echo "<li><b>Error: There is no database support in your PHP setup</b><br />\n";
...
if ($f_mysql_connect == 1) {
    echo "<li>Depends on: MySQL 3.23, 4.0 - OK</li>\n";
}
...
if ($phpversion >= 5) {
    if ($f_mysqli_connect == 1) {
        echo "<li>Depends on: MySQL 4.1 - OK\n";
        if (!($config_loaded && $CONF['database_type'] == 'mysqli')) {
            echo "<br>(change the database_type to 'mysqli' in config.local.php if you want to use MySQL)\n";
        }
        echo '</li>';
    }
}
...
//
if ($f_pg_connect == 1) {
    echo "<li>Depends on: PostgreSQL - OK \n";
    if (!($config_loaded && $CONF['database_type'] == 'pgsql')) {
        echo "<br>(change the database_type to 'pgsql' in config.local.php if you want to use PostgreSQL)\n";
    }
    echo '</li>';
}

if ($f_sqlite_open == 1) {
    echo "<li>Depends on: SQLite - OK \n";
    if (!($config_loaded && db_sqlite())) {
        echo "<br>(change the database_type to 'sqlite' in config.local.php if you want to use SQLite)\n";
    }
    echo '</li>';
}
...
if ($config_loaded) {
    ...
    if ($error_text == '') {
        echo "<li>Testing database connection (using {$CONF['database_type']}) - OK</li>";
    } else {
        echo "<li><b>Error: Can't connect to database</b><br />\n";
        echo "Please edit the \$CONF['database_*'] parameters in config.local.php.\n";
        echo "$error_text</li>\n";
        ++$error;
    }
}
...
if ($f_session_start == 1) {
    echo "<li>Depends on: session - OK</li>\n";
} else {
    echo "<li><b>Error: Depends on: session - NOT FOUND</b><br />\n";
...
//
if ($f_preg_match == 1) {
    echo "<li>Depends on: pcre - OK</li>\n";
} else {
    echo "<li><b>Error: Depends on: pcre - NOT FOUND</b><br />\n";
...
if ($f_mb_encode_mimeheader == 1) {
    echo "<li>Depends on: multibyte string - OK</li>\n";
} else {
    echo "<li><b>Error: Depends on: multibyte string - NOT FOUND</b><br />\n";
...
//
if ($f_imap_open == 1) {
    echo "<li>Depends on: IMAP functions - OK</li>\n";
} else {
    echo "<li><b>Warning: Depends on: IMAP functions - NOT FOUND</b><br />\n";
...
if ($error != 0) {
    echo '<p><b>Please fix the errors listed above.</b></p>';
} else {
...
<div class="standout"><?php echo $setupMessage; ?></div>
<div id="edit_form">
...
<table>
   ...
   <tr>
      <td><label for="username"><?php echo $PALANG['admin'] . ':'; ?></label></td>
      <td><input id="username" class="flat" type="text" name="username" value="<?php echo $tUsername; ?>" /></td>
      <td><?php echo _error_field($errormsg, 'username'); ?> <?php echo $PALANG['email_address'] ?></td>
   </tr>
   <tr>
       <td><label for="password"><?php echo $PALANG['password'] . ':'; ?></label></td>
      <td><input id="password" class="flat" type="password" name="password" /></td>
      <td><?php echo _error_field($errormsg, 'password'); ?></td>
   </tr>
   <tr>
       <td><label for="password2"><?php echo $PALANG['password_again'] . ':'; ?></label></td>
      <td><input id="password2" class="flat" type="password" name="password2" /></td>
      <td><?php echo _error_field($errormsg, 'password2'); ?></td>
   </tr>
   <tr>
